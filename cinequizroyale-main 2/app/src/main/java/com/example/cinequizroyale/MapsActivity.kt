...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    private fun setupTitle() {
        try {
            ...
            titleText.text = "Cinemas"
            titleText.textSize = 20f
            titleText.setTextColor(resources.getColor(R.color.primary_text, null))
            // Set the font programmatically
            val typeface = ResourcesCompat.getFont(this, R.font.luckiest_guy)
            titleText.typeface = typeface

            Log.d(tag, "Title set up successfully")
        } catch (e: Exception) {
            Log.e(tag, "Error setting up title: ${e.message}", e)
        }
    }
    ...
    private fun displayCinemaInfo(cinema: Cinema) {
        try {
            ...
            try {
                // Debug log cinema info
                Log.d(tag, "Cinema: ${cinema.name}, Movies: ${cinema.currentMovies.joinToString(", ")}")
                // Set the cinema details in the card
                cinemaNameText.text = cinema.name
    ...
    private fun setupBackButton() {
        try {
            ...
            backButton.text = "←"
            backButton.textSize = 44f
            backButton.setTextColor(resources.getColor(R.color.primary_text, null))
    ...
    private inner class MoviesAdapter(private val movies: List<String>) :
        ...
        override fun onBindViewHolder(holder: MovieViewHolder, position: Int) {
            try {
                ...
                // Debug log all movie titles
                Log.d(tag, "Movie at position $position: '$movieTitle'")

                // Try to find exact match first
                var imageResource = movieImageMap[movieTitle]

                // If no exact match, try case-insensitive match
                if (imageResource == null) {
                    val matchingKey = movieImageMap.keys.firstOrNull {
                        it.equals(movieTitle, ignoreCase = true)
                    }

                    if (matchingKey != null) {
                        imageResource = movieImageMap[matchingKey]
                        Log.d(tag, "Found case-insensitive match: '$matchingKey' for '$movieTitle'")
                    } else {
                        // Try fuzzy matching by removing special characters
                        val normalizedTitle = movieTitle.replace(Regex("[\\s-:&']"), "").lowercase()
                        val matchingFuzzyKey = movieImageMap.keys.firstOrNull {
                            it.replace(Regex("[\\s-:&']"), "").lowercase() == normalizedTitle
                        }

                        if (matchingFuzzyKey != null) {
                            imageResource = movieImageMap[matchingFuzzyKey]
                            Log.d(tag, "Found fuzzy match: '$matchingFuzzyKey' for '$movieTitle'")
                        } else {
                            Log.d(tag, "No matching image found for movie: '$movieTitle'")
                        }
                    }
                }

                holder.moviePoster.setImageResource(imageResource ?: R.drawable.tardes_de_soledad)
            } catch (e: Exception) {
